...
class VideoLink{
	...
	**/
	function _56( $vid ) {
		...
		$json = $json['info'];
		$r['vid'] = $json['textid'];
		$r['url'] = 'http://www.56.com/u/v_'. $json['textid'] .'.html';
	...
	**/
	function sohu( $vid ) {
		...
		$r['vid'] = $vid;
		$r['url'] = $json['url'];
		$r['swf'] = 'http://share.vrs.sohu.com/my/v.swf&id=' . $vid;
	...
	function _163($url){
			...
			//http://swf.ws.126.net/openplayer/v01/-0-2_M6GGG3N1E_M6GGGVILR-vimg1_ws_126_net//image/snapshot_movie/2010/11/P/6/M6KCF1HP6-.swf
			$pattern = '/(http:\/\/swf\.ws\.126\.net\/openplayer\/.*\.swf)/is';
