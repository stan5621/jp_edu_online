		...
		<table style="width:100%">
			<tr>
				...
				<td>
					...
					<div style="margin-bottom:5px;"><?php echo CHtml::link($post->user->name,array('//u/index','id'=>$post->user->id),array('class'=>'dxd-username','data-userId'=>$post->user->id));?><span class="muted">&nbsp;&nbsp;<?php echo date("Y-m-d H:i",$post->addTime);?></span>
					...
					<?php if($member->inRoles(array('superAdmin','admin'))):?>
						<?php echo CHtml::link(($post->isTop>0?'取消置顶':'置顶'),array('group/setTopPost','id'=>$group->id,'postId'=>$post->id,'value'=>($post->isTop>0?'0':'1')),array('class'=>'muted'));?>
					<?php endif;?>
					&nbsp;|&nbsp;
					<?php 
					if(Yii::app()->user->checkAccess('updatePost',array('post'=>$post))){
						 echo CHtml::link('编辑',array('post/update','id'=>$post->id),array('class'=>'muted'));
					}
					?>
					&nbsp;|&nbsp;
				<?php if(Yii::app()->user->checkAccess('deletePost',array('post'=>$post)) || $member->inRoles(array('superAdmin','admin'))):?>
						<?php echo CHtml::link('删除',array('group/deletePost','id'=>$group->id,'postId'=>$post->id),array('class'=>'muted'));?>
				<?php endif;?>
					</div>
