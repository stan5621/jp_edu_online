...
class LessonController extends CaController
{
	...
	public function actionSetMedia($lessonId){
		...
		if(isset($_POST['mediaId'])){
			$name = $_POST['name'];
			//$lesson->mediaType = (CFileHelper::getMimeTypeByExtension($name)?CFileHelper::getMimeTypeByExtension($name):"video/mp4");
	...
	/*
	 public function actionAddLecture($id){
		$lesson = $this->loadModel($id);
		$lesson->mediaType = "lecture";
		$lesson->save();
		$this->redirect(array('index','courseId'=>$lesson->courseId));
		}

		public function actionAddQuiz($id){
		$quiz = new Quiz;
		$lesson = $this->loadModel($id);
		if($quiz->save()){
		...
		$lesson->save();
		Yii::app()->user->setFlash('success','添加测试成功');
		}else{
		Yii::app()->user->setFlash('error','抱歉，添加测试失败');
		}
		$this->redirect(array('index','courseId'=>$lesson->courseId));
		}
		*/
	...
	public function actionCreateMany($courseId){
		...
		if(isset($_POST['playList'])){
			...
			foreach($videos as $item){
				...
				if($mediaLink->save()){
					$lesson = new Lesson;
					$lesson->title = $mediaLink->title ?  $mediaLink->title : "未设置";
					$lesson->mediaType = "link";
			...
			if($result) Yii::app()->user->setFlash('success','操作成功！');
			else Yii::app()->user->setFlash('error','操作失败！');
		}
