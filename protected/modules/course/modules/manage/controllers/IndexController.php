...
class IndexController extends CaController
{
	...
	public function actionSetBasic($id)
	{
		...
		if(isset($_POST['Course'])){
			...
			Yii::app()->user->setFlash('success','保存信息成功！');
				...
				Yii::app()->user->setFlash('error','抱歉！保存信息失败！');
			}
		}
	...
	public function actionEditMember($id,$userId){
		...
		if(isset($_POST['CourseMember'])){
			...
			if($member->save()) Yii::app()->user->setFlash('success','保存成功！');
			else Yii::app()->user->setFlash('error','保存失败！');
		}
		$this->layout = "/layouts/nonav_column1";
		if($member){
			$myMember = $course->findMember(array('userId'=>Yii::app()->user->id));
			if(Yii::app()->user->checkAccess('admin')){
				$roleItems = array('superAdmin'=>'超级管理员','admin'=>'管理员','teacher'=>'人员','student'=>'学员');
			}else if ($myMember->inRoles(array('superAdmin'))) {
				$roleItems=array('admin'=>'管理员','teacher'=>'人员','student'=>'学员');
			}else if($myMember->inRoles(array('admin'))){
				$roleItems=array('teacher'=>'人员','student'=>'学员');
			}
	...
	public function actionAddMember($id){
		if(isset($_POST['CourseMember'])){
			...
			if(!$user) {
				Yii::app()->user->setFlash('error','抱歉！该用户不存在！');
			}else{
				$model->userId = $user->id;
				$model->startTime = time();
				$model->courseId = $id;
				if(CourseMember::model()->findByAttributes(array('userId'=>$user->id,'courseId'=>$model->courseId))){
					Yii::app()->user->setFlash('error','该名成员已经存在！');
				}else if($model->save()){
					Yii::app()->user->setFlash('success','添加成功');
				}else{
					Yii::app()->user->setFlash('error','抱歉，添加失败');
				}
			}
