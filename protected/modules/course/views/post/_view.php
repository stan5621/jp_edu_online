			...
			<tr>
				...
				<td>
					...
					<?php if($member->inRoles(array('superAdmin','admin'))):?>
						<?php echo CHtml::link(($post->isTop>0?'取消置顶':'置顶'),array('setTop','id'=>$post->id,'value'=>($post->isTop>0?'0':'1')),array('class'=>'muted'));?>
					<?php endif;?>
					&nbsp;|&nbsp;
					<?php 
				...
				<?php if(Yii::app()->user->checkAccess('deletePost',array('post'=>$post)) || $member->inRoles(array('superAdmin','admin'))):?>
						<?php echo CHtml::link('删除',array('delete','id'=>$course->id,'postId'=>$post->id),array('class'=>'muted'));?>
				<?php endif;?>
					</div>
