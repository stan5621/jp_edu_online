<div class="dxd-course-header">	
	<div class="dxd-course-nav" style="border-top:none;padding-top:15px;padding-bottom:15px;">
		...
		<div class="pull-left dxd-course-header-left2">
			...
			&nbsp;<?php echo round($course->rateScore,1);?>分
			&nbsp;&nbsp;&nbsp;<?php echo CHtml::link('('.$course->rateNum.'条评价)',array('rates','id'=>$course->id),array('class'=>'dxd-fancy-elem','data-boxHeight'=>"800"));?>&nbsp;&nbsp;&nbsp;&nbsp;
			<?php if(! $course->hasRated(Yii::app()->user->id,$course->id)) echo CHtml::link('我要评价',array('toggleRate','id'=>$course->id),array('class'=>'dxd-fancy-elem'));?>
			    ...
			    <?php $this->widget('bootstrap.widgets.TbButtonGroup', array(
			       // 'type'=>'primary', // '', 'primary', 'info', 'success', 'warning', 'danger' or 'inverse'
			        'buttons'=>array(
			   		    array('label'=>'收藏', 'url'=>'#'),
			            array('items'=>array(
			                array('label'=>($course->isCollector(Yii::app()->user->id)?'取消个人收藏':"个人收藏"), 'url'=>array('index/toggleCollect','id'=>$course->id)),
			                array('label'=>'小组收藏', 'url'=>array('//group/course/myGroup','courseId'=>$course->id),'linkOptions'=>array('class'=>'dxd-fancy-elem')),
			            ),
			...
			<div class="mr15" style="margin-top:80px">
				<?php 
					if(!(Yii::app()->user->checkAccess('admin')) && !($member->inRoles(array('superAdmin','admin'))) && $member && !$member->isValid()):?>
						<!--  <span class="text-error">已过有效期，请重新<?php echo CHtml::link('购买','#');?></span> -->
						<span class="text-error">已过有效期，<?php echo CHtml::link('点击续费￥'.$course->renewFee,array('rebuy','id'=>$course->id),array('class'=>'text-error'));?></span>
				<?php endif;?>
			</div>
...
<div class="clearfix"></div>
  ...
  <?php $this->widget('bootstrap.widgets.TbMenu', array(
    ...
    'items'=>array(
        array('label'=>'主页', 'url'=>array('/course/index/view','id'=>$course->id)),
        array('label'=>'讨论区', 'url'=>array('/course/post/index','courseId'=>$course->id)),
        array('label'=>'进度', 'url'=>array('/course/index/progress','id'=>$course->id)),
    ),
