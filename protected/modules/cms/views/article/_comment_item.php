...
<td>
<div style="margin-bottom:5px;"><?php echo CHtml::link($data->user->name,array("u/index",'id'=>$data->user->id),array('class'=>'dxd-username','data-userId'=>$data->user->id));?><span class="muted">&nbsp;&nbsp;<?php echo date("Y-m-d H:i",$data->addTime);?></span>
...
<?php if($data->refer){?>
    <div class="dxd-post-comment-refer"><?php echo $data->refer->user->name;?>:<?php $output=strip_tags($data->refer->content);echo mb_strlen($output)>160 ? mb_substr($output,0,160,'utf8')."...":$output;?></div>
        <span style="font-weight:bold;color:#000;">回复</span><?php echo CHtml::link($data->refer->user->name.":","#",array('style'=>'font-weight:bold;color:#000;'));?>				
        <?php }?>	
        <?php echo $data->content;?>
        </div>
        <div style="text-align:right" class="muted">
        <?php echo CHtml::link('回复','javascript:;',array('class'=>'muted','data-toggle'=>'collapse','data-target'=>'#dxd-comment-form'.$index));?>&nbsp;|
        <?php  
        if(Yii::app()->user->checkAccess('admin')) 
        echo CHtml::link('删除',array('deleteComment','id'=>$model->id,'commentId'=>$data->id),array('class'=>'muted'));?>
        </div>
        <div id="dxd-comment-form<?php echo $index;?>" class="collapse">
        <?php /** @var BootActiveForm $form */
        $form = $this->beginWidget('CActiveForm',array('action'=>array('addComment','id'=>$model->id))); ?>
